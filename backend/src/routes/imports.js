...
router.get('/google/preview',authorizeRoles('admin','operator'),async(_request,response,next)=>{try{response.json(await new SyncService().previewWorkbook())}catch(error){next(error)}})
